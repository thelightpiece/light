...
class BanktransferController extends Controller
{
    ...
    /**
     * Store or update bank transfer payment settings.
     * * This method handles validation for the bank account number, sanitizes the input 
     * by trimming whitespace, and updates the workspace settings. If the payment 
     * gateway is toggled off, it automatically clears the bank account number.
     *
    ...
    public function setting(Request $request)
    {
        if ($request->has('bank_transfer_payment_is_on')) {
            ...
            $bank_transfer = [
                'bank_transfer_payment_is_on' => $request->bank_transfer_payment_is_on,
                'bank_number'                 => trim($request->bank_number), // Ditambahkan trim agar nomor rekening bersih dari spasi liar
            ];
            ...
            // Jika status dimatikan, update status ke 'off' sekaligus kosongkan nomor rekening di database
            $bank_transfer_off = [
                'bank_transfer_payment_is_on' => 'off',
                'bank_number'                 => '',
            ];

            foreach ($bank_transfer_off as $key => $value) {
                $data = [
                    'key'        => $key,
                    'workspace'  => getActiveWorkSpace(),
                    'created_by' => creatorId(),
